...
if ($dari && $sampai) {
    $stmt = $db->prepare("SELECT m.*, a.nama_barang FROM maintenances m JOIN assets a ON m.asset_id = a.id WHERE m.tanggal_maintenance BETWEEN ? AND ? ORDER BY m.id DESC");
...
<body>
    <div class="print-wrapper">
        ...
        <div class="judul-laporan">
            <h3>LAPORAN DATA MAINTENANCE</h3>
        </div>
        ...
        <?php if (empty($maintenances)): ?>
            <p style="text-align:center;padding:40px 0;color:#666;font-size:11pt;">Tidak ada data maintenance</p>
        <?php else: ?>
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?= sanitize($m['nama_barang']) ?></td>
                        <td class="text-center"><?= formatTanggal($m['tanggal_maintenance']) ?></td>
                        <td class="text-center"><?= sanitize($m['jenis'] ?? '-') ?></td>
                        <td><?= sanitize($m['deskripsi'] ?? '-') ?></td>
                        <td><?= sanitize($m['teknisi'] ?? '-') ?></td>
                        <td class="text-right"><?= formatRupiah($m['biaya'] ?? 0) ?></td>
                        <td class="text-center"><?= sanitize($m['status'] ?? '-') ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
