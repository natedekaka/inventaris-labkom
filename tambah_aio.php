...
echo "<h1>Tambah 12 AIO Lenovo Hitam</h1>";
...
if ($cat && $cat->num_rows > 0) {
    ...
    echo "<p>✅ Kategori <strong>PC Desktop</strong> sudah ada (ID: $cat_id)</p>";
} else {
...
$loc_id = null;
$loc = $db->query("SELECT id FROM locations WHERE LOWER(nama_lokasi) = 'lab 1' LIMIT 1");
if ($loc && $loc->num_rows > 0) {
    $loc_id = $loc->fetch_assoc()['id'];
    echo "<p>✅ Lokasi <strong>LAB 1</strong> sudah ada (ID: $loc_id)</p>";
} else {
...
$last = $db->query("SELECT kode_aset FROM assets WHERE kode_aset LIKE 'PC%' ORDER BY id DESC LIMIT 1");
$nextNum = 1;
...
echo "<h2 style='color:green;margin:0 0 12px 0;'>✅ BERHASIL! $inserted dari 12 aset ditambahkan</h2>";
echo "<table border='1' cellpadding='6' style='border-collapse:collapse;'>";
echo "<tr><th>No</th><th>Kode</th><th>Nama</th><th>Merek</th><th>Model</th><th>Kategori</th><th>Lokasi</th></tr>";
for ($i = 0; $i < $inserted; $i++) {
    $k = sprintf('PC-%04d', $nextNum + $i);
    echo "<tr><td>" . ($i+1) . "</td><td>$k</td><td>AIO Lenovo Hitam</td><td>Lenovo</td><td>F0D8002KID</td><td>PC Desktop</td><td>LAB 1</td></tr>";
}
echo "</table>";
echo "<p style='margin-top:16px;'><a href='/assets/?search=lenovo' style='display:inline-block;padding:10px 24px;background:#1a1a2e;color:#fff;text-decoration:none;border-radius:4px;font-weight:bold;'>Lihat di Aset →</a></p>";
echo "</div>";
